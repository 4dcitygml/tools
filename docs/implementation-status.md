...
- Cities beyond PLATEAU (tools after tools-v1.2.2): the schema gate validates CityGML 1.0 as well as 2.0; the commit scope and minimal-diff gates identify buildings by the city's `building_id` (`uro:buildingID`, `gml:id` or a generic attribute such as `gen:BIN`) and locate members in the file's own dialect; the topology gate projects from the city's `crs` and leaves metric projections as they are
